...
class LoginPage(object): 
    ...
    def createPage(self): 
        ...
        Label(self.page, text='用户登录', font=("华文行楷", 20), fg="#86967e",bg="#fffaf4").grid(row=0, stick=W) 
        Label(self.page, text = '用户名: ',bg="#fffaf4").grid(row=1, stick=W, pady=10) 
        Entry(self.page, textvariable=self.username).grid(row=1, column=1, stick=E) 
        Label(self.page, text = '密码: ',bg="#fffaf4").grid(row=2, stick=W, pady=10) 
    ...
    def loginCheck(self): 
        ...
        id = Identity().checkID(name)
        print(id)
        Loginstate().update(id[0]['cardID'])
        if value[0]['root'] == 0: 
            self.page.destroy() 
            MainPage(self.root) 
        elif value[0]['root'] == 1:
