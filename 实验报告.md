...
我在本学期学习了密码学课程，刚了解到md5加密算法，为保护用户隐私，我在后端获取到用户输入时使用md5算法对其密码进行加密再存入数据库中，保证了用户密码的安全性。就算数据库出现隐私泄露的情况用户密码依旧是安全的。
...
- pack
	- 优势：可以默认居中或从某一方向开始排布，对于大型部件来说非常便于布局。
...
3. 报错信息如下：
	...
	发现是在list中的值错误了，输出list调试发现是因为传参的时候，没有传递正确格式，将格式改为：
...
5. 在很多此收到返回值时，没有有效的传入sql

	解决方案：发现自己对python中的字典，列表，元组的定义不熟练，在了解了详细定义之后还是很顺利的拿出了里面的值。
...
支持通过四种条件进行查询，最终结果返回其交集。

<img src="实验报告.assets/image-20220416005747382.png" alt="image-20220416005747382" style="zoom:67%;" />
